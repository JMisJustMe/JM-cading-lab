...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        Button mount = new Button(this);
        mount.setText("Body");
        mount.setAllCaps(false);
        mount.setOnClickListener(v -> chooseBody());
        bar.addView(mount, new LinearLayout.LayoutParams(dp(this, 78), dp(this, 44)));
        ...
        web.setWebViewClient(new WebViewClient() {
            @Override public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest request) {
                ...
                try {
                    ...
                    Toast.makeText(MainActivity.this, "No app can open this route", Toast.LENGTH_SHORT).show();
                }
                return true;
            }
        });
    ...
    private void chooseBody() {
        Intent intent = new Intent(Intent.ACTION_OPEN_DOCUMENT)
                ...
                .setType("text/html");
        startActivityForResult(intent, MOUNT_BODY);
    }
    ...
    private void loadMountedBodyOrWelcome() {
        ...
        String html = "<!doctype html><meta name=viewport content='width=device-width,initial-scale=1'>"
                + "<style>body{margin:0;background:#07131f;color:#eef6ff;font:17px system-ui;padding:28px}"
                + ".box{max-width:680px;margin:auto;background:#102438;border:1px solid #31516c;border-radius:24px;padding:24px}"
                + "button{background:#6eddba;border:0;border-radius:16px;padding:15px 18px;font-weight:800;font-size:17px}"
                + "code{color:#9ce8d0}</style><div class=box><h1>JM Estate Storage Steward</h1>"
                + "<h2>v0.9B · Native Border</h2><p>Mount the exact Steward HTML body once. The source remains unchanged; this app keeps a private working copy and gives its exports stable Android custody.</p>"
                + "<button onclick='JMNative.openBodyPicker()'>Mount Steward HTML</button>"
                + "<p>Exports will be retained in <code>Downloads/JM Estate Storage Steward</code>.</p></div>";
    ...
    private void mountBody(Uri source) {
        try {
            ...
            if (staged.length() < 64) throw new IOException("Selected body is empty or incomplete");
            byte[] probe = new byte[(int)Math.min(staged.length(), 4096)];
            try (InputStream input = new FileInputStream(staged)) {
                int read = input.read(probe);
                String head = new String(probe, 0, Math.max(read, 0), StandardCharsets.UTF_8).toLowerCase(Locale.ROOT);
                if (!head.contains("<html") && !head.contains("<!doctype") && !head.contains("<head")) {
                    throw new IOException("Selected file does not look like an HTML body");
                }
            }
            ...
            Toast.makeText(this, "Steward HTML mounted privately", Toast.LENGTH_SHORT).show();
            loadMountedBodyOrWelcome();
        } catch (Exception e) {
        ...
        @JavascriptInterface public String saveText(String requested, String text, String mime) {
            ...
            try {
                ...
                JSONObject row = new JSONObject()
                        .put("name", name).put("uri", uri.toString())
    ...
    @Override protected void onDestroy() {
        if (web != null) {
            web.removeJavascriptInterface("JMNative");
            web.destroy();
        }
        super.onDestroy();
    }
