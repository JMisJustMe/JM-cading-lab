#!/usr/bin/env python3
"""Prove exact two-build Android release reproducibility and inspectable APK surfaces."""
...
def file_inventory(root: Path, *, excluded_dirs: set[str] | None = None) -> list[dict[str, Any]]:
...
def _attribute_string(lines: list[str], attribute: str) -> str | None:
    for line in lines:
...
def _exported_value(lines: list[str]) -> bool | None:
    for line in lines:
...
def parse_components(xmltree: str) -> list[dict[str, Any]]:
    ...
    for index, line in enumerate(lines):
        ...
        for later in lines[index + 1 :]:
            element = element_pattern.match(later)
...
def verify_exported_surface(components: list[dict[str, Any]], expected_activity: str) -> None:
    ...
    item = exported[0]
    if item["kind"] != "activity" or item["name"] != expected_activity:
        raise RuntimeError(
            f"unexpected exported surface: expected activity {expected_activity}, recovered {item}"
        )
...
def toolchain_receipt(
    ...
    *,
    ...
    gradle_text = release.run([gradle, "--version"], timeout=120).stdout
    java_result = release.run(["java", "-version"], timeout=120)
    java_text = "\n".join(part for part in (java_result.stdout, java_result.stderr) if part)
    aapt2_text = release.run([str(aapt2), "version"], timeout=120).stdout
    apksigner_text = release.run([str(apksigner), "version"], timeout=120).stdout
    return {
        "gradle_version": gradle_text.strip(),
        "gradle_version_sha256": text_sha256(gradle_text),
        "java_version": java_text.strip(),
        "java_version_sha256": text_sha256(java_text),
        "aapt2_version": aapt2_text.strip(),
        "aapt2_sha256": release.file_sha256(aapt2),
        "apksigner_version": apksigner_text.strip(),
...
def prove_one(
    ...
    *,
    ...
    source_inventory = file_inventory(project, excluded_dirs={"build", ".gradle"})
    ...
    with tempfile.TemporaryDirectory(prefix=f"jm-assurance-{body_id}-") as temporary:
        ...
        if signed_a_hash != signed_b_hash:
            raise RuntimeError(
                f"same-key signed reproducibility drift for {body_id}: "
        ...
        components = parse_components(xmltree)
        verify_exported_surface(components, expected_activity)

        inventory_a = apk_inventory(signed_a)
        inventory_b = apk_inventory(signed_b)
        if inventory_a != inventory_b:
            raise RuntimeError(f"signed APK entry inventory drift for {body_id}")
        paths = [item["path"] for item in inventory_a]
        required = {"AndroidManifest.xml", "classes.dex", "resources.arsc", "assets/body.json"}
        ...
        if missing:
            raise RuntimeError(f"release APK inventory missing for {body_id}: {missing}")
        if native_entries:
            raise RuntimeError(f"unexpected native payload for {body_id}: {native_entries}")
        if private_entries:
            raise RuntimeError(f"private key payload inside APK for {body_id}: {private_entries}")
        ...
        receipt = {
            ...
            "apk_inventory": inventory_a,
            "apk_inventory_count": len(inventory_a),
            "apk_inventory_sha256": inventory_digest(inventory_a),
            ...
            "claim_boundary": (
                ...
                "byte-identical after same-key alignment/signing on one runner/toolchain. Compiled "
                "permissions, exported components, APK inventory, native payload and private-key "
                "surface were also receipted. Cross-runner, cross-OS, production-key and store "
                "reproducibility remain separate gates."
            ),
        }
        write_json(receipts / f"{body_id}.json", receipt)

    print(f"JM_ANDROID_RELEASE_ASSURANCE_PASS:{body_id}:{receipt['canonical_apk_sha256']}", flush=True)
    return receipt
...
def build_shard(
    ...
    *,
    ...
    root = out / (
        f"assurance-smoke-{body_id}"
        if body_id
        else f"assurance-shard-{shard_index:02d}-of-{shard_count:02d}"
    )
    if root.exists():
        shutil.rmtree(root)
    ...
    batch = {
        ...
        "unsigned_byte_reproducibility_proofs": sum(
            bool(item["unsigned_byte_reproducible"]) for item in passed
        ),
        "signed_byte_reproducibility_proofs": sum(
            bool(item["same_key_signed_byte_reproducible"]) for item in passed
        ),
        "zipalign_proofs": sum(bool(item["zipalign_verified"]) for item in passed),
        ...
        "failures": failed,
        "claim_boundary": (
            "This shard proves exact two-build reproducibility and inspectable release surfaces on "
            "one runner/toolchain. Cross-runner and production-signing reproducibility remain open."
        ),
    }
