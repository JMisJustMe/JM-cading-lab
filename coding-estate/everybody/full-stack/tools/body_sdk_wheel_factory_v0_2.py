#!/usr/bin/env python3
"""Build and retain all compiler/LSP/debugger/bytecode/VM body SDK wheels."""
...
SCHEMA = "jm.everybody.sdk-wheel-factory/0.2"
...
def metadata_from_wheel(path: Path) -> tuple[dict[str, str], str, str, set[str]]:
...
def build_one(sdk_root: Path, wheel_dir: Path, log_dir: Path) -> dict[str, Any]:
    ...
    wheel = created.pop()
    metadata, entry_points, package, names = metadata_from_wheel(wheel)
    ...
    commands = [f"jm-{body_id}-{suffix}" for suffix in command_suffixes]
    for command in commands:
        if command not in entry_points:
            raise RuntimeError(f"entry point {command} missing from {wheel.name}")
    for required in (
        f"{package}/__init__.py",
        f"{package}/compiler.py",
        f"{package}/language_server.py",
        f"{package}/debugger.py",
        f"{package}/bytecode.py",
        f"{package}/vm.py",
        f"{package}/core.py",
        f"{package}/tooling_core.py",
        f"{package}/vm_core.py",
        f"{package}/py.typed",
    ):
...
def build(source: Path, out: Path) -> dict[str, Any]:
    ...
    for ordinal, sdk_root in enumerate(sdk_roots, 1):
        ...
        write_json(receipt_dir / f"{entry['body_id']}.json", {"schema": "jm.body.sdk-wheel/0.2", **entry})
        entries.append(entry)
        print(f"[{ordinal}/{EXPECTED_BODY_COUNT}] JM_SDK_WHEEL_VM_PASS:{entry['body_id']}:{entry['wheel_sha256']}", flush=True)
    ...
    manifest = {
        ...
        "commands_per_body": COMMANDS_PER_BODY,
        "command_count": EXPECTED_BODY_COUNT * COMMANDS_PER_BODY,
        "entries": entries,
        "claim_boundary": "Wheel construction proves installable hosted SDK/VM packaging. Public registry publication and native self-hosting remain separate gates.",
    }
    write_json(out / "00_OPEN_FIRST_BODY_SDK_VM_WHEEL_RECEIPT.json", manifest)
    (out / "README.md").write_text(
        "# JM EveryBody Sovereign SDK + VM Wheels v0.2\n\n"
        "Contains one identity-bound installable wheel for each of the first 100 sovereign coding bodies. "
        "Each wheel contains its compiler, SDK API, language server, debugger, bytecode compiler, verifier and VM.\n",
        encoding="utf-8",
    )
