...
def patch(path: Path, replacements: list[tuple[str, str]]) -> None:
    ...
    for old, new in replacements:
        if old not in text:
            raise SystemExit(f"PATCH HOLD: expected source seam missing in {path}: {old!r}")
        text = text.replace(old, new)
    path.write_text(text, encoding="utf-8")
    print(f"PATCHED {path.relative_to(ROOT)}")


# Frozen v1.3 files remain unchanged in Git. This script modifies only the ephemeral
# CI checkout to create the authorised 105-body forward parity descendant.
patch(
...
patch(
    ...
    [
        (
            "All first-100 bodies have body-specific capability effects and isolated process runtimes.",
            "All 105 admitted bodies have body-specific capability effects and isolated process runtimes.",
        ),
    ],
)
