...
printf '%s\n' '<!doctype html><meta charset="utf-8"><title>JM3232 Navigator</title><h1>JM3232 Navigator — Public Read-Only Plugin Descendant</h1><p>Production MCP endpoint: <code>/mcp</code></p>' > "$OUT/index.html"
...
let x=await rpc(1,'initialize',{protocolVersion:'2025-06-18',capabilities:{},clientInfo:{name:'JM plugin public proof',version:'1'}}); if(!x.r.ok||x.j.error)throw new Error(JSON.stringify(x.j));
...
x=await rpc(3,'tools/call',{name:'search',arguments:{query:'Radius Lexicon',limit:1}}); if(!x.r.ok||!x.j.result.structuredContent.results.length)throw new Error('public search failed');
x=await rpc(4,'tools/call',{name:'navigator_create_stringreceipt',arguments:{action:'NOPE',target_id:'X',outcome:'NOPE'}}); if(x.r.ok||!x.j.error)throw new Error('write tool unexpectedly exposed');
console.log('public read-only MCP local proof PASS');
NODE
...
for A in $(seq 1 15); do
  ...
  if [[ "$CODE" == '200' ]] && grep -q PUBLIC_PLUGIN_READ_ONLY_READY /tmp/nav-plugin-alias-health.json; then PUBLIC_BASE="$ALIAS"; break; fi
  sleep 2
done
export PUBLIC_BASE
...
async function rpc(id,method,params={}){const r=await fetch(base+'/mcp',{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id,method,params})});const j=await r.json();return {r,j};}
let x=await rpc(1,'initialize',{protocolVersion:'2025-06-18',capabilities:{},clientInfo:{name:'JM public plugin proof',version:'1'}}); if(!x.r.ok||x.j.error)throw new Error(JSON.stringify(x.j)); const init=x.j.result;
x=await rpc(2,'tools/list'); if(!x.r.ok||x.j.result.tools.length!==5)throw new Error('tool count'); const tools=x.j.result.tools;
x=await rpc(3,'resources/list'); if(!x.r.ok||x.j.result.resources.length!==1)throw new Error('resource count'); const resources=x.j.result;
...
x=await rpc(5,'tools/call',{name:'navigator_create_stringreceipt',arguments:{action:'NOPE',target_id:'X',outcome:'NOPE'}}); if(x.r.ok||!x.j.error)throw new Error('write exposure');
