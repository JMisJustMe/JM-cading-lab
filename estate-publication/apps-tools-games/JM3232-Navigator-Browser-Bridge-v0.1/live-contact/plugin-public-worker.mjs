const APP_VERSION = "0.1.0-plugin-public.2";
...
const TOOLS = [
  ...
  { name:"navigator_bridge_status", title:"Inspect bridge status", description:"Inspect public source counts and the live publication boundary without exposing owner runtime state or changing Navigator state.", inputSchema:{type:"object",properties:{},additionalProperties:false}, outputSchema:OUTPUT_OBJECT, annotations:ro, _meta:toolMeta() },
];
...
function sanitizeStructured(name,v){
  ...
  if(name==="navigator_bridge_status") return {name:"JM3232 Navigator",version:APP_VERSION,state:"PUBLIC_PLUGIN_READ_ONLY_MINIMIZED_READY",source_counts:v?.source_counts||{},boundary:"Public submission descendant: five read-only retrieval tools; no owner runtime state, write tools, raw source excerpts, or shared mutable user state are exposed."};
  return v;
}
...
async function handleMcp(request){
  ...
  try{
    if(method==="initialize"){
      ...
      return json(rpcResult(id,{protocolVersion:protocol,capabilities:{tools:{listChanged:false}},serverInfo:{name:"JM3232 Navigator — Public Read-Only Plugin Descendant",version:APP_VERSION},instructions:"Search before fetch. Preserve source identity. Public responses are deliberately minimized. This descendant exposes five read-only retrieval tools only; owner write-state and raw source excerpts remain outside the public Plugin."}));
    }
    ...
    if(method==="tools/call"){
      ...
      return json(rpcResult(id,sanitizeToolResult(params.name,result)));
    }
...
export default {
  async fetch(request,env){
    ...
    if(url.pathname==="/health") return json({ok:true,name:"JM3232 Navigator",version:APP_VERSION,state:"PUBLIC_PLUGIN_READ_ONLY_MINIMIZED_READY",source_mcp:SOURCE_MCP,tool_count:TOOLS.length,resource_count:0,shared_mutable_user_state:false,raw_source_excerpts:false});
