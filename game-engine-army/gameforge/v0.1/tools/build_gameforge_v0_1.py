...
def build(source: Path, out: Path) -> dict[str, object]:
    ...
    single = index.replace(
        ...
        '<script src="app-loader.js"></script>', f"<script>\n{javascript}\n</script>"
    )
    single_path = out / "OPEN_FIRST_GAMEFORGE_SOVEREIGN_REBUILD_v0_1.html"
    ...
    receipt: dict[str, object] = {
        "schema": "jm.gameforge-build-receipt/0.1",
        "status": "GAMEFORGE_SOVEREIGN_REBUILD_STATIC_BUILD_PASS_BROWSER_CONTACT_OPEN",
        "source_parts": PARTS,
        "source_part_count": len(PARTS),
        "javascript_sha256": sha256(javascript.encode("utf-8")),
        "single_file": single_path.name,
        "single_file_sha256": sha256(single_path.read_bytes()),
        "game_body_count": len(manifest["gameBodies"]),
        "coding_body_count": len(manifest["codingBodies"]),
