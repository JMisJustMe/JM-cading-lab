...
PARTS=['app.part1.txt','app.part2.txt','app.part3a.txt','app.part3b.txt','app.part4.txt']
...
def build(source:Path,out:Path)->dict:
 ...
 target=out/'OPEN_FIRST_GLYPHFORGE_SOVEREIGN_REBUILD_v0_1.html';target.write_text(single)
 ...
 r={'schema':'jm.glyphforge-build-receipt/0.1','status':'GLYPHFORGE_SOVEREIGN_REBUILD_STATIC_BUILD_PASS_BROWSER_CONTACT_OPEN','source_part_count':len(PARTS),'javascript_sha256':sha(js.encode()),'single_file':target.name,'single_file_sha256':sha(target.read_bytes()),'target_count':len(m['adapterTargets']),'mode_count':len(m['controlModes']),'donor_count':len(m['donors']),'claim_boundary':m['claimBoundary']};r['receipt_sha256']=sha(json.dumps(r,sort_keys=True,separators=(',',':')).encode());(out/'BUILD_RECEIPT.json').write_text(json.dumps(r,indent=2,sort_keys=True)+'\n');return r
def main():
