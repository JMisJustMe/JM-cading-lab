...
assert(html.includes('PUKA v0.13A'),'current v0.13A front-door marker missing');
assert(html.includes('puka-v12.css'),'v0.12 full-table presentation layer must remain linked');
assert(html.includes('puka-v13.css'),'v0.13 Deep House presentation layer missing');
...
assert(sw.includes("const CACHE='jm-puka-v13a'"),'service worker cache must advance with current descendant');
assert(sw.includes("'./puka-v12.css'"),'v0.12A presentation layer must remain cached');
assert(sw.includes("'./puka-v13.css'"),'v0.13A presentation layer must be cached');
assert(registry.includes('JM.PUKA.Room/0.13A'),'current registry schema missing');
...
assert(receipt.includes('DEEP HOUSE VISUAL CONVERGENCE BUILD RECEIPT'),'current v0.13 build receipt missing');
assert(receipt.includes('v0.12 active-hand continuity'),'build receipt must preserve the learning-loop parent capability');

console.log('PUKA v0.12A FULL TABLE SOURCE DING PASS INSIDE CURRENT v0.13A DESCENDANT');
