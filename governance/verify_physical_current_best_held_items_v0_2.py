...
def verify(root: Path) -> dict[str, object]:
    ...
    expect(held.get("status") == "TWO_TARGETS_HELD_AND_GOVERNED", "held register status drift")
    ...
    expect("not located in the accessible GitHub and indexed Library sweep" in boundary, "accessible-scope boundary missing")

    return {
        "status": "PASS_TWO_HELD_TARGETS_GOVERNED",
        "held_count": 2,
        "resolved_count": 0,
        "targets": [
        ...
        "next_gate": "Exact bytes plus target-specific exit criteria",
    }
