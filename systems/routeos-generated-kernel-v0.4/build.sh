#!/usr/bin/env bash
set -euo pipefail
ROOT="$(cd "$(dirname "$0")" && pwd)"; BASE="$ROOT/../routeos-two-generated-bodies-v0.3"; BUILD="$ROOT/build"; GENK="$BUILD/generated_kernel"; GENU="$BUILD/generated_users"
rm -rf "$BUILD"; mkdir -p "$GENK" "$GENU"
test ! -e "$ROOT/kernel/routeos_kernel.c"; test ! -e "$ROOT/arch/x86_64/boot.S"; test ! -e "$ROOT/linker.ld"
node "$ROOT/compiler/generate_routeos_kernel.mjs" "$ROOT/source/routeos_kernel_blueprint.jm.cading" "$GENK"
...
python3 "$ROOT/tests/verify_generated_kernel.py" "$ROOT" "$BASE" "$BUILD/routeos-kernel.elf"
printf 'GENERATED_KERNEL_BUILD PASS %s\n' "$BUILD/routeos-kernel.elf"
