...
extern char routeos_generated_body_source_sha[];
extern char routeos_generated_body_ir_sha[];
...
static void *jm_memcpy(void *dst, const void *src, size_t n) {
  uint8_t *d = (uint8_t *)dst;
  const uint8_t *s = (const uint8_t *)src;
  for (size_t i = 0; i < n; ++i) d[i] = s[i];
  return dst;
}
static void *jm_memset(void *dst, int value, size_t n) {
  uint8_t *d = (uint8_t *)dst;
...
static void serial_u64(uint64_t value) {
  ...
  while (value && i < sizeof(b)) { b[i++] = (char)('0' + value % 10); value /= 10; }
  while (i) serial_char(b[--i]);
}
static void receipt(const char *message) {
  serial_write("[JM] ");
...
struct cpu_frame *routeos_interrupt_dispatch(struct cpu_frame *frame) {
  ...
  if (frame->vector == 128) {
    ...
    if (active->frame.rax == SYS_ROUTE_STATE) {
      routeos_state = active->frame.rdi;
      serial_write("[JM] GENERATED USER BODY "); serial_u64(active->id);
      serial_write(" -> SYSTEM CALL ROUTE_STATE -> PERMISSIONGATE PASS -> ROUTEOS STATE ");
      serial_u64(routeos_state);
      serial_write(" -> KERNEL RETURN\n");
      active->frame.rax = routeos_state;
      active->state = BODY_READY;
      return &active->frame;
    }
...
static void user_boundary_install(void) {
  ...
  size_t user1_blob_size = (size_t)(routeos_user1_blob_end - routeos_user1_blob_start);
  size_t user2_blob_size = (size_t)(routeos_user2_blob_end - routeos_user2_blob_start);
  jm_memcpy((void *)USER1_CODE, routeos_user1_blob_start, user1_blob_size);
  jm_memcpy((void *)USER2_CODE, routeos_user2_blob_start, user2_blob_size);
...
__attribute__((noreturn)) void routeos_kernel_entry(uint32_t magic, uint32_t mb_info) {
  ...
  serial_write("[JM] ONEBODY GENERATED USER BODY 1 LOADED: SOURCE_SHA256 ");
  serial_write(routeos_generated_body_source_sha);
  serial_write(" IR_SHA256 ");
  serial_write(routeos_generated_body_ir_sha);
  serial_write("\n");
