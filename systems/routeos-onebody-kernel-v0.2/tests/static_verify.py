...
entry_dis = subprocess.check_output(
    ['objdump', '-d', '--disassemble=routeos_kernel_entry', str(p)], text=True
)
assert not re.search(r'\bsti\b', entry_dis), (
    'premature STI remains in routeos_kernel_entry; user IRETQ must enable IF atomically'
)
...
print('ONEBODY_GENERATED_SYMBOL_GATE PASS')
