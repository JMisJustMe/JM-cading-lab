...
rm -f "$TRACE"
set +e
timeout --signal=TERM --kill-after=2 12 qemu-system-x86_64 -machine pc,accel=tcg -m 128M -cdrom "$ISO" -boot d -display none -serial stdio -monitor none -no-reboot -no-shutdown 2>&1 | tee "$TRACE"
qemu_status=${PIPESTATUS[0]}
set -e
if [[ "$qemu_status" -ne 0 && "$qemu_status" -ne 124 ]]; then echo "HOLD: QEMU exited unexpectedly with status $qemu_status"; exit "$qemu_status"; fi
python3 "$ROOT/tests/verify_trace.py" "$TRACE"
printf 'TWO_GENERATED_BODIES_DING PASS (capture_status=%s)\n' "$qemu_status"
