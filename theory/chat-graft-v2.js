...
(() => {
  const V1_URL = './data/chat-graft-v1.b64?v=1100';
  ...
  async function loadSources() {
    const [v1Text, v2Parts] = await Promise.all([
      fetchText(V1_URL),
      Promise.all(Array.from({ length: V2_PARTS }, (_, index) => (
        fetchText(`${V2_BASE}${String(index + 1).padStart(2, '0')}.txt?v=1100`)
      )))
    ]);
  ...
  function updateSummary() {
    ...
    resultHead.insertAdjacentHTML('beforeend', '<span class="chat-graft-summary">115 source-backed placeholders · 145 still open</span>');
  }
    ...
    reader.insertAdjacentHTML('beforeend', `
      <section class="chat-graft-box">
        ...
        <div class="graft-meta">
          <span class="graft-tag">${esc(graft.match_state)}</span>
  ...
  loadSources()
    ...
    .catch((error) => console.error('JM theory chat-source graft v2:', error));
})();
